...
pub fn config_override_summary_lines(cfg: &LoadedConfig) -> Vec<String> {
    ...
    if env.contains_key("CCP_CODEX_INLINE_COMPACTION_THRESHOLD") {
        out.push("CCP_CODEX_INLINE_COMPACTION_THRESHOLD (env)".to_string());
    }
    if env.contains_key("CCP_CODEX_COMPACTION_STATE_DIR") {
        out.push("CCP_CODEX_COMPACTION_STATE_DIR (env)".to_string());
    }
    if env.contains_key("CCP_CODEX_SESSION_MAX_CONCURRENT_REQUESTS") {
        out.push("CCP_CODEX_SESSION_MAX_CONCURRENT_REQUESTS (env)".to_string());
    }
    if env.contains_key("CCP_CODEX_SESSION_QUEUE_TIMEOUT_SECS") {
        out.push("CCP_CODEX_SESSION_QUEUE_TIMEOUT_SECS (env)".to_string());
    }
    if env
