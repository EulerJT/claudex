...
pub fn assert_allowed_model(model: &str) -> Result<(), ModelNotAllowedError> {
    if model == CODEX_AUTO_REVIEW_MODEL || ALLOWED_MODELS.contains(&model) {
        Ok(())
    } else {
...
mod tests {
    ...
    fn allowed_models_accept_base() {
        assert!(assert_allowed_model("codex-auto-review").is_ok());
        assert!(assert_allowed_model("gpt-5.4").is_ok());
        assert!(assert_allowed_model("gpt-5.6").is_ok());
