...
pub struct ResponsesRequest {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub context_management: Option<Vec<ResponsesContextManagement>>,
    /// Exact JSON fragments used only for the HTTP wire body. This lets the
    /// inline compaction layer replay opaque and future output item types
    /// byte-for-byte without weakening the typed request translator.
    #[serde(skip)]
    pub raw_input_override: Option<Vec<String>>,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(tag = "type", rename_all = "snake_case")]
pub enum ResponsesContextManagement {
...
impl ResponsesRequest {
    pub fn wire_value(&self) -> Result<Value, serde_json::Error> {
        ...
        if let Some(items) = self.raw_input_override.as_ref() {
            let input = items
                .iter()
            ...
            value
                .as_object_mut()
                .expect("ResponsesRequest serializes as an object")
                .insert("input".to_string(), Value::Array(input));
        }
        Ok(value)
    }

    pub fn wire_json_string(&self) -> Result<String, serde_json::Error> {
        let Some(items) = self.raw_input_override.as_ref() else {
            return serde_json::to_string(self);
        };

        let raw_input = serde_json::value::RawValue::from_string(format!("[{}]", items.join(",")))?;
    ...
    match name {
        "Read" => {
            let base = description
                .unwrap_or_else(|| "Reads a file from the local filesystem.".to_string());
            Some(format!("{base}\n\n{}", read_offset_guidance()))
        }
        "Grep" => {
            let base = description.unwrap_or_else(|| "Searches file contents.".to_string());
            Some(format!("{base}\n\n{}", grep_argument_guidance()))
        }
        _ => description,
    }
}
...
fn grep_argument_guidance() -> &'static str {
    "Claudex Grep argument guidance:\n\
     - The field \"A\" is not valid.\n\
     - Use \"context\" when surrounding matching lines are needed.\n\
     - Use only fields defined in this schema."
}
...
mod tests {
    ...
    #[test]
    fn translate_grep_tool_hardens_unknown_argument_contract_without_strict_mode() {
        let req: MessagesRequest = serde_json::from_value(json!({
            "model": "gpt-5.5",
        ...
        let description = tool.description.as_deref().unwrap();
        assert!(description.starts_with("Search file contents."));
        assert!(description.contains("The field \"A\" is not valid."));
        assert!(description.contains("Use \"context\""));
        assert_eq!(
