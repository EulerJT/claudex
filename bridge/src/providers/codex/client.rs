...
pub fn build_websocket_request(
    ...
    let mut payload = body.wire_value().unwrap_or_default();
...
impl CodexHttpClient {
    ...
    pub async fn stream_codex_http_events(
        ...
        let body_json = body.wire_json_string().map_err(|e| CodexError {
            status: 500,
                ...
                let mut retry_error = 'read_attempt: loop {
                    ...
                    let chunk = match chunk {
                        ...
                        Ok(Err(err)) => {
                            ...
                            if !semantic_output_forwarded && pending_terminal.is_none() {
                                break 'read_attempt error;
                            }
                            let _ = tx.send(Err(error)).await;
                            return;
                    ...
                    for event in events {
                        ...
                        if pending_terminal.is_some() {
                            let message = if terminal {
                                "Codex SSE stream emitted duplicate terminal response events"
                            } else {
                                "Codex SSE stream emitted a payload after its terminal response event"
                            };
                            let error = http_sse_error(message);
                            log_http_stream_end(
                                &log,
                                "codex_http_stream_failed",
                                &req_id,
...
mod tests {
    ...
    async fn finish_http_chunks(stream: &mut tokio::net::TcpStream) {
    ...
    #[tokio::test]
    async fn http_stream_sends_raw_input_override_unchanged_across_retry() {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let addr = listener.local_addr().unwrap();
        let server = tokio::spawn(async move {
            ...
            for attempt in 0..2 {
                ...
                let header_end = request
                    ...
                    .position(|part| part == b"\r\n\r\n")
                    .unwrap();
                bodies.push(request[header_end + 4..].to_vec());
                if attempt == 0 {
                    stream
                        .write_all(
                            b"HTTP/1.1 503 Service Unavailable\r\ncontent-length: 0\r\nretry-after: 0\r\nconnection: close\r\n\r\n",
                    ...
                    let response = b"data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_replay\",\"status\":\"completed\",\"usage\":{}}}\n\n";
                    let head = format!(
                        "HTTP/1.1 200 OK\r\ncontent-type: text/event-stream\r\ncontent-length: {}\r\nconnection: close\r\n\r\n",
                        response.len()
                    );
        ...
        let client = Arc::new(http_test_client(format!("http://{addr}/responses"), 1_000));
        client.auth_manager().set_test_auth(http_test_auth());
        let mut request = buffered_request_with_texts(&["full-history-must-not-reach-upstream"]);
        request.raw_input_override = Some(vec![
            r#"{"type":"compaction","encrypted_content":"opaque-live"}"#.to_string(),
            r#"{"type":"message","role":"user","content":[{"type":"input_text","text":"next"}]}"#
                .to_string(),
        ]);
        let mut events = client
            .stream_codex_http_events(&request, &http_test_context())
            .await
            .unwrap();

        assert_eq!(events.recv().await.unwrap().unwrap()["type"], "keepalive");
        assert_eq!(
            events.recv().await.unwrap().unwrap()["type"],
            "response.completed"
        );
        ...
        assert_eq!(bodies[0], bodies[1], "retry must reuse the exact wire body");
        let body: serde_json::Value = serde_json::from_slice(&bodies[0]).unwrap();
        assert_eq!(body["input"][0]["type"], "compaction");
        assert_eq!(body["input"][0]["encrypted_content"], "opaque-live");
        assert_eq!(body["input"][1]["content"][0]["text"], "next");
        assert!(
            !String::from_utf8_lossy(&bodies[0]).contains("full-history-must-not-reach-upstream")
        );
    }

    #[tokio::test]
    async fn http_stream_rejects_duplicate_terminal_before_forwarding_it() {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let addr = listener.local_addr().unwrap();
        let server = tokio::spawn(async move {
            let (mut stream, _) = listener.accept().await.unwrap();
            ...
            let body = concat!(
                ...
                "data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_1_duplicate\",\"status\":\"completed\",\"usage\":{}}}\n\n"
            );
            let head = format!(
                "HTTP/1.1 200 OK\r\ncontent-type: text/event-stream\r\ncontent-length: {}\r\nconnection: close\r\n\r\n",
                body.len()
        ...
        assert!(error.message.contains("duplicate terminal"));
        assert!(events.recv().await.is_none());
        server.await.unwrap();
    }
