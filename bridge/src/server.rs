...
const CODEX_AUTO_REVIEW_EFFORT: &str = "low";
...
fn apply_auto_review_model(
    ...
    if route.override_model == CODEX_AUTO_REVIEW_MODEL {
        ...
        output_config
            .as_object_mut()
            .expect("output_config normalized to an object")
            .insert(
                "effort".to_string(),
                Value::String(CODEX_AUTO_REVIEW_EFFORT.to_string()),
            );
    }
    Some(route)
}
...
pub async fn serve_listener(
    ...
    let _inline_compaction_writer_lock =
        crate::providers::codex::inline_compaction::acquire_state_dir_writer_lock_from_environment(
        )
        .map_err(|error| anyhow::anyhow!(error))?;
    if let Some(writer_lock) = _inline_compaction_writer_lock.as_ref() {
        let capability = writer_lock.capability();
        create_logger("codex_inline_compaction_state").info(
            "state_backend_capability_confirmed",
            Some(serde_json::Map::from_iter([
                (
                    "filesystemType".to_string(),
                    json!(&capability.filesystem_type),
                ),
                ("mountId".to_string(), json!(capability.mount_id)),
                ("mountOptions".to_string(), json!(&capability.mount_options)),
                (
                    "directoryFsyncSupported".to_string(),
                    json!(capability.directory_fsync_supported),
                ),
            ])),
        );
    }
...
async fn dispatch_request(
    ...
    let provider = if auto_review_route
        .as_ref()
        .is_some_and(|route| route.override_model == CODEX_AUTO_REVIEW_MODEL)
        && original_provider
            .as_ref()
            .is_some_and(|provider| provider.name() == "codex")
    {
        original_provider.clone()
    } else if auto_review_route.is_some() {
    ...
    if let Some(route) = auto_review_route.as_ref() {
        let review_effort = crate::providers::translate_shared::read_effort(&body)
            .ok()
            .flatten();
        log.info(
            ...
            Some(Map::from_iter([
                ...
                ("reviewEffort".to_string(), json!(review_effort)),
            ])),
        );
    }
    ...
    if status.is_success() {
        return monitor_response_body_with_log(
            response,
            request_guard,
...
fn monitor_response_body(response: Response, guard: RequestMonitorGuard) -> Response {
    monitor_response_body_inner(response, guard, None)
}

fn monitor_response_body_with_log(
    response: Response,
    guard: RequestMonitorGuard,
    completion_log: RequestCompletionLog,
) -> Response {
    monitor_response_body_inner(response, guard, Some(completion_log))
}

fn monitor_response_body_inner(
    response: Response,
    guard: RequestMonitorGuard,
    completion_log: Option<RequestCompletionLog>,
) -> Response {
    let status = response.status();
    ...
    let stream = futures_util::stream::unfold(
        (body, guard, outcome, completion_log),
        move |(mut body, mut guard, outcome, mut completion_log)| async move {
            match body.frame().await {
                Some(Ok(frame)) => Some((Ok(frame), (body, guard, outcome, completion_log))),
                Some(Err(err)) => {
                    guard.failed(status, err.to_string());
                    if let Some(completion_log) = completion_log.take() {
                        completion_log.record(status);
                    }
                    Some((Err(err), (body, guard, outcome, completion_log)))
                }
                None => {
...
mod auto_review_tests {
    ...
    #[test]
    fn codex_classifier_defaults_to_native_auto_review_model() {
        ...
        assert_eq!(route.override_model, "codex-auto-review");
        assert_eq!(classifier.model.as_deref(), Some("codex-auto-review"));
        assert_eq!(classifier.extra["output_config"]["effort"], "low");
    }

    #[test]
    fn native_auto_review_forces_low_effort_and_preserves_output_format() {
        let mut classifier = request(
            "You are a security monitor for autonomous AI coding agents.",
            false,
            json!([]),
        ...
        apply_auto_review_model(&mut classifier, false, None, "codex")
            .expect("classifier should be routed");

        assert_eq!(classifier.extra["output_config"]["effort"], "low");
        assert_eq!(
            classifier.extra["output_config"]["format"]["type"],
            "json_schema"
        );
    }
    ...
    fn configured_model_overrides_provider_default() {
        ...
        assert!(classifier.extra.get("output_config").is_none());
    }

    #[test]
    fn configured_model_accepts_codex_native_auto_review_route() {
        let mut classifier = request(
            "You are a security monitor for autonomous AI coding agents.",
            false,
            json!([]),
        ...
        let route =
            apply_auto_review_model(&mut classifier, false, Some("codex-auto-review"), "codex")
                .expect("configured classifier should use the native Codex review model");
        assert_eq!(route.override_model, "codex-auto-review");
        assert_eq!(classifier.model.as_deref(), Some("codex-auto-review"));
    }
